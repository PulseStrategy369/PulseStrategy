...
/**
 * @title PulseStrategy
 * @notice A Decentralized vPLS (Vouch-staked PulseChain) Reserve, allows xBond, iBond, and their liquidity providers to claim PLStr as a reward, redeemable for vPLS at anytime. vpls deposits create claims for plstr based on reserve ratio. depositors do not recive plstr.
 * @dev PLStr has a 0.5% burn on transfers (excluding claims and redemptions). LP receive 2x PLStr. Claims expire after 90 days.
 */
contract PulseStrategy is ERC20, ReentrancyGuard {
    ...
    address private immutable _xBond = 0xe81AB1BFaa001EaE4f226F8a8D3d607dC76D5b0A;
    address private immutable _iBond = 0xb67073Fe8F2A41807E038b1F91572bEad3950c84;
    ...
    address private immutable _xBondPlsxLP = 0xd1d0e58A8f575CC27F11827c1e42c76a305Ca5B9;
    address private immutable _iBondIncLP = 0x86183a0ABd2dDeD0CFa55D2a6655A8dB71e71A06;

    // --------------------------------------
    // Constants
    // --------------------------------------
    uint256 private constant _MIN_DEPOSIT = 10000e18; // 10,000
    uint256 private constant _MIN_TRANSFER = 10e15; // 0.01
    uint256 private constant _CLAIM_PRECISION = 1e18; // 1
    uint256 private constant _WEIGHT_COOLDOWN = 86400; // 24hrs
    uint256 private constant _INITIAL_IBOND_WEIGHT = 2559614466000000000;
    uint256 private constant _BURN_FEE = 50; //0.5%
    uint256 private constant _FEE_DENOMINATOR = 10000; // 10,000
    uint256 private constant _LP_MULTIPLIER = 2; // 2x
    uint256 private constant _EXPIRATION_PERIOD = 90 days; // 90 days
    uint256 private constant _SECONDS_PER_DAY = 86400; // 24hrs
    ...
    function claimPLStr() external nonReentrant {
        ...
        uint256 totalEligibleSupply = _getTotalEligibleSupply();
        if (totalEligibleSupply == 0) revert ZeroTokenSupply();
        uint256 weightedBalance = _getWeightedBalance(msg.sender);
        uint256 proportionalReward = _totalClaimablePLStr.mulDiv(weightedBalance, totalEligibleSupply, Math.Rounding.Floor);
        reward = reward > proportionalReward ? proportionalReward : reward;
        if (reward == 0) revert NoEligibleTokens();
        _userRewards[msg.sender] = 0;
        _totalClaimablePLStr = reward > _totalClaimablePLStr ? 0 : _totalClaimablePLStr - reward;
